...
def trainWeights(trainingDatasetX, trainingDatasetY):
    ...
    for epochIndex in range(1, env.MAX_EPOCHS):
        ...
        weightsByTime[epochIndex] = w.getDecayedWeights(
            weightsByTime[epochIndex-1])
        ...
        for patternIndex, pattern in enumerate(trainingDatasetX):
            ...
            # NOTE: Delta weights are are only added to the most transient weight. Unless the memory_size of that memory type is set to zero (i.e., is effectively switched 'off'), in which case the memory type above it will be used.
            memorySizes = np.array([item['memory_size'] for item in env.SYNAPSE_MEMORY_TYPES.values()])
            indexOfLastNonZeroMemorySize = np.max(np.nonzero(memorySizes))


            deltaWeights = np.zeros(weightsByTime[epochIndex].shape)
            deltaWeights[:, indexOfLastNonZeroMemorySize] = env.LEARNING_RATE * \
                (error * pattern)

            # Override current weights according to prediction from decayed weights.
            weightsByTime[epochIndex] = w.updateWeights(
                weightsByTime[epochIndex], deltaWeights, neuronalTypes)

            # Override current weights with new values after checking for consolidations.
            weightsByTime[epochIndex], consolidationsByTime[epochIndex] = w.consolidateWeightsAboveThreshold(
                weightsByTime[epochIndex], weightsByTime[epochIndex-1], consolidationsByTime[epochIndex])
