...
def evaluatePerformance(actualTargets, predictedTargets):
  ...
  truePositives = np.where((actualTargets == 1.0) &
                        (predictedTargets == 1.0))[0].size
  falsePositives = np.where((actualTargets == 1.0) &
                         (predictedTargets == -1.0))[0].size
  trueNegatives = np.where((actualTargets == -1.0) &
                        (predictedTargets == -1.0))[0].size
  falseNegatives = np.where((actualTargets == -1.0) &
                         (predictedTargets == 1.0))[0].size
